...
回路図
-----------------
この実験では、アクティブブザー、NPNトランジスタ、および1kΩの抵抗を使用します。抵抗は、GPIOピンとトランジスタのベースの間に配置され、トランジスタを保護します。Raspberry PiのGPIO17が低レベル（0V）に設定されると、トランジスタが飽和して導通し、ブザーが鳴ります。GPIO17が高レベルに設定されると、トランジスタはオフになり、ブザーは鳴りません。
